/**
 * 画像生成専用 — 株式会社ワム公式HP参照ルール
 *
 * 公式HPは Knowledge Base のみ（商品情報・商品画像・ブランドルール）。
 * デザイン（レイアウト・配色・タイポ）は HP から取らず、毎回 AI が新規設計する。
 */

import {
  WAM_OFFICIAL_SITE,
  WAM_PRODUCT_INDEX,
  resolveProductFromAnswers,
  getProductImageMode,
} from "./wamProducts.js";

/** 画像生成の絶対ルール（全画像プロンプトに適用） */
export const WAM_IMAGE_GENERATION_RULES = [
  "【最重要】商品画像はAIで新規生成・再デザイン・改変しない",
  "公式HPから取得するのは商品情報・商品画像URL・ブランドルールのみ（Knowledge Base）",
  "公式HPのページレイアウト・配色・タイポグラフィ・構図は再現禁止",
  "背景・人物・レイアウト・配色・装飾・タイポグラフィは毎回AIがゼロから新規設計",
  "商品名・ロゴ・パッケージは公式情報と完全一致（商品画像のみ公式利用）",
  "公式に存在しない商品ビジュアルを創作しない。不明な属性は出力に含めない",
  "HPスクリーンショット風・Webページ風の見た目にしない",
  "毎回異なるオリジナル販促クリエイティブを目指す（テンプレート使い回し禁止）",
];

/** @param {Object<string,string>} answers */
export function buildWamProductBlock(answers) {
  const product = resolveProductFromAnswers(answers);
  const mode = getProductImageMode(product);

  const lines = [
    "【Knowledge Base — 公式HP参照範囲】",
    `- 参照元: ${WAM_PRODUCT_INDEX}`,
    "- 取得対象: 商品情報・商品画像URL・ブランドルールのみ",
    "- 取得禁止: HPのレイアウト・配色・タイポ・ページデザイン",
  ];

  if (!product) {
    lines.push(
      "",
      "【商品画像の扱い — 商品なしモード】",
      "- 商品画像は一切生成・配置しない",
      "- 背景・人物・装飾・レイアウト・配色・タイポはAIが毎回新規設計",
      "- 架空の商品・パッケージ・ロゴを創作しない"
    );
    return lines.join("\n");
  }

  lines.push(
    "",
    "【対象商品 — 公式掲載情報（Knowledge Base）】",
    `- 商品名: ${product.name}`,
    `- カテゴリ: ${product.category}`,
    `- 公式説明: ${product.description}`,
    `- 公式ページ: ${product.officialUrl}`
  );

  if (mode === "official") {
    lines.push(
      "",
      "【商品画像 — 公式画像をそのまま使用】",
      `- 公式商品画像URL: ${product.officialImageUrl}`,
      "- このURLの商品画像を加工・再生成せず配置する",
      "- 商品本体の形状・色・ロゴ・ラベル・パッケージは1ピクセルも変更しない",
      "- 背景・人物・装飾・レイアウト・配色・タイポは毎回AIがオリジナル設計",
      "- 公式HPのページデザイン・配色・タイポは再現しない"
    );
  } else {
    lines.push(
      "",
      "【商品画像 — 公式HPに掲載画像なし / 取得不可】",
      `- ${product.name} の公式商品画像は、現時点でアプリから参照可能なURLがありません`,
      "- 商品画像をAIで生成してはならない",
      "- ユーザーに公式商品画像または正規パッケージ写真のアップロードを依頼する",
      "- アップロード画像が提供されるまで: 背景・人物・装飾・レイアウト・配色・タイポのみ新規設計"
    );
  }

  if (answers.product_image_upload) {
    lines.push("", "【ユーザー提供画像】", `- ${answers.product_image_upload}`);
  }

  return lines.join("\n");
}

/** @param {Object<string,string>} answers */
export function buildImageGenerationInstructions(answers) {
  const product = resolveProductFromAnswers(answers);
  const mode = getProductImageMode(product);

  const sections = [
    buildWamProductBlock(answers),
    "",
    "【生成指示の構造 — オリジナルクリエイティブ】",
    "以下の順で出力すること:",
    "1. オリジナルクリエイティブコンセプト（HP再現禁止・毎回新規設計）",
    "2. 背景・人物・装飾・配色・タイポグラフィの生成プロンプト（英語）",
  ];

  if (mode === "official") {
    sections.push(
      "3. 公式商品画像の配置指示（URL指定・配置位置・加工禁止の明記）",
      "4. コピー文案（日本語）"
    );
  } else if (mode === "upload_required") {
    sections.push(
      "3. 【必須】ユーザーへの商品画像アップロード依頼文",
      "4. 商品画像なし状態でのオリジナルシーン生成プロンプト（英語）",
      "5. コピー文案（日本語）— 商品名は公式表記のみ使用"
    );
  } else {
    sections.push(
      "3. オリジナルシーン生成プロンプト（英語）— 商品要素は含めない",
      "4. コピー文案（日本語）"
    );
  }

  sections.push(
    "",
    `【用途】${answers.usage || answers.sns_format || ""}`,
    `【サイズ・比率】${answers.aspect || ""}`,
    `【訴求】${answers.message || answers.appeal_axis || ""}`,
    `【掲示場所】${answers.target || answers.display_location || ""}`,
    `【スタイル】${answers.style || ""}`
  );

  if (answers.extra_info) {
    sections.push(`【補足（ユーザー入力）】${answers.extra_info}`);
  }

  return sections.join("\n");
}

/** 画像プロンプト専用フッター */
export function getImagePromptFooter() {
  return `【画像生成 最終チェック】
- 商品画像をAIで新規生成・改変していないか
- 公式HPのレイアウト・配色・タイポを再現していないか
- 公式HPにない商品名・ロゴ・パッケージを創作していないか
- 公式商品画像は加工せず配置する指示になっているか
- 背景・人物・レイアウト・配色・タイポはオリジナル設計になっているか
- 毎回異なるクリエイティブになっているか（テンプレート使い回しでないか）`;
}
